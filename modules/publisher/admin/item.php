...
switch ($op) {
    ...
    case 'del':
        ...
        if ($confirm) {
			...
			// İLAVE ALANLARI SİL
include_once XOOPS_ROOT_PATH . '/modules/customfields/include/functions.php';
customfields_deleteData('publisher', $itemId);
...
function publisher_editItem($showmenu = false, $itemId = 0, $clone = false)
{
    ...
    // İLAVE ALANLAR - Template'e assign et
    include_once XOOPS_ROOT_PATH . '/modules/customfields/include/functions.php';
    $currentItemId = $itemObj->itemid();
    
    // Form HTML'ini buffer'a al
    ob_start();
    echo customfields_renderForm('publisher', $currentItemId);
    $customFieldsHTML = ob_get_clean();
    
    // Template'e gönder
    $formTpl->assign('customfields_form', $customFieldsHTML);
